...
def search_youtube(query: str, max_results: int = 6) -> Optional[VideoResult]:
    ...
    Strategy order is optimised for Streamlit Cloud where DDGS google
    backend consistently returns "No results found" (network restriction).
    ddgs.videos() succeeds reliably and is tried first.
    ...
    """
    strategies = [
        # 1. DDGS native video search — works on Streamlit Cloud, fast
        ("ddgs_videos",  lambda: _search_via_ddgs_videos(query, max_results)),
        # 2. DDGS auto backend text search — broader fallback
        ("ddgs_auto",    lambda: _search_via_ddgs_auto(query, max_results)),
        # 3. DDGS google backend — works locally, often blocked on cloud
        ("ddgs_google",  lambda: _search_via_ddgs_google(query, max_results)),
        # 4. youtube-search-python — pure scrape, last resort
        ("ytsearch",     lambda: _search_via_ytsearch(query)),
    ]
    ...
    for name, strategy in strategies:
        try:
            ...
            logger.warning("Search strategy '%s' raised: %s", name, exc)
